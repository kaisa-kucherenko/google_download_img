...
def is_contain_string(base_string, sub_string):
    base_string = base_string.lower()
    if base_string.startswith(f'{sub_string}+'):  # at start of string
        return True
    elif base_string.endswith(f'+{sub_string}'):  # at end of string
        return True
    elif f'+{sub_string}+' in base_string:  # at middle of string
        return True
    else:
        return False
...
def find_imgs_urls(browser, search_query, img_number, fc_club):
    if fc_club:
        if '/' in search_query:
            ...
            search_query = search_query.replace("&", "and").replace(" ", "+")
        else:
            search_query = search_query.replace(" ", "+")
    else:
        search_query = search_query.replace(" ", "+")
    # if search_query has 'fc' (begin, middle, end of string) - do not add fc
    if not is_contain_string(search_query, 'fc'):
